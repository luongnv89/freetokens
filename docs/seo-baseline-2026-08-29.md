...
**Audit command:** `python3 scripts/audit_seo.py --json app/dist` passed. The issue's illustrative `--max-files 200` spelling is now supported as an ergonomic cap (`--max-files 300` via `npm run audit:seo`); it is unnecessary for this three-page run because the script itself limits the audit to `dist/*.html`. The result covered 3 files with 0 warnings and 0 info findings. For contributors the filtered command is `cd app && npm run audit:seo` (strictly `app/dist`), which excludes `.tsx`/`.lighthouseci/` noise — see § Audit-noise filtering below.
...
## Audit-noise filtering (Task 3.6)

**Historical false-positive class:** Early ad-hoc audits that scanned the repository tree (e.g. `grep -r` over `app/src/**/*.tsx` or a flat `**/*.html` without a `dist/` prefix) reported inflated counts in the 121/171 range. Those hits were component source and Lighthouse CI artifacts (`.tsx` files containing `og:title`-like strings, `.lighthouseci/` reports, `app/src` templates) rather than the shipped static pages. The baseline `scripts/audit_seo.py` intentionally limits itself to `dist/*.html` (sorted top-level HTML only; `dist/offers/*.html` is outside the baseline scope) so the signal reflects what search crawlers see.

**Correct filter:** `python3 scripts/audit_seo.py app/dist` from the repo root, or the contributor shorthand `cd app && npm run audit:seo` which runs `python3 ../scripts/audit_seo.py dist --max-files 300` strictly against `app/dist`. The `--max-files` flag is an ergonomic cap for large builds and does not change the `dist/*.html` filter; it simply truncates after N files. CI can invoke `npm run audit:seo` after `npm run build` without pulling in `app/src` or `.lighthouseci/` noise, and a `--fail-on-critical` gate can be added once the deferred JSON-LD/robots/sitemap findings are resolved.

**Current signal on green main (post-Task 3.6):** The filtered `dist/*.html` audit reports 0 criticals for the required gate (one canonical, one `og:title`, viewport, `lang` per page) and, after the head-or-body JSON-LD fix, 0 overall criticals when `robots.txt`/`sitemap.xml` are present. The `.tsx` 121/171 class is documented here as non-shipped noise, not a dist regression.
