...
| 4 | Verification filter | PASS | 38 × `badge-verification-social_proof`, 4 × `hand_verified` badges rendered; verification dimension part of the three-dimension filter (#126) with URL state covered in `urlState.test.ts`. |
...
| 15 | Live traffic strip | PASS | `TrafficStrip.tsx` renders `#ft-traffic` footer line (`role="status"`, hidden until analytics grants consent), matching `build.py #ft-traffic`; production shows the same marker. |
