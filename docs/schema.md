...
| `category`      | enum (string)         | yes      | no          | One of: `api_provider`, `coding`, `image`, `voice`, `video`, `startup_program`, `student`. |
...
- **Category enum:** exactly one of
  `api_provider | coding | image | voice | video | startup_program | student`.
