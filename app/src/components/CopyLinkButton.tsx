...
export function CopyLinkButton({ url }: { url: string }) {
  ...
  function confirm(ok: boolean) {
    setStatus(ok ? "Link copied!" : "Copy failed — long-press the address bar instead.")
  }
  ...
  return (
    <section className="od-share" aria-label="Copy link to this offer">
      ...
      <p className="share-status" id="ft-share-status" role="status" aria-live="polite" hidden={!status}>
        {status}
      </p>
    </section>
  )
