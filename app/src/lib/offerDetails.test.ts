...
describe("claimSteps", () => {
  ...
  it("returns the document's claim_steps when present", () => {
    expect(claimSteps({ claim_steps: ["One.", "Two."] })).toEqual([
      "One.",
      "Two.",
    ]);
  });
...
describe("offerMetaDescription", () => {
  ...
  it("passes through a short summary unchanged", () => {
    expect(offerMetaDescription(offer, { summary: "Short blurb." })).toBe(
      "Short blurb.",
    );
  });
  ...
  it("truncates at 160 chars with a 157-char prefix, rstrip, and ellipsis", () => {
    ...
    expect(offerMetaDescription(offer, { summary: "B".repeat(160) })).toBe(
      "B".repeat(160),
    );
  });
});
...
describe("resolveAsset", () => {
  it("prefixes site-relative paths for depth-1 pages and leaves others alone", () => {
    ...
    expect(resolveAsset("./assets/shot.png")).toBe("./assets/shot.png");
    expect(resolveAsset("/assets/shot.png")).toBe("/assets/shot.png");
    expect(resolveAsset("https://cdn.example/shot.png")).toBe(
...
describe("isRenderableProof / required keys", () => {
  ...
  it("rejects entries missing required keys or using a bad screenshot path", () => {
    expect(
      isRenderableProof({ type: "x", url: "https://x.com/a", author: "Ada" }),
    ).toBe(false);
    expect(
      isRenderableProof({
        type: "reddit",
    ...
    expect(
      isRenderableProof({ type: "screenshot", image: "assets/x.png" }),
    ).toBe(false);
    expect(
      isRenderableProof({
        type: "screenshot",
    ...
    expect(
      isRenderableProof({ type: "link", url: "https://example.com" }),
    ).toBe(false);
    expect(
      isRenderableProof({
        type: "unknown",
