...
function sitemapEntries(xmlText) {
  ...
  return [...document.getElementsByTagNameNS(SITEMAP_NAMESPACE, "url")].map((url) => ({
    loc: url.getElementsByTagNameNS(SITEMAP_NAMESPACE, "loc")[0].textContent,
    lastmod: url.getElementsByTagNameNS(SITEMAP_NAMESPACE, "lastmod")[0].textContent,
  }))
}
...
describe("sitemap generation", () => {
  ...
  it("clamps future dates and falls back to a page mtime", () => {
    ...
    expect(entries.every(({ lastmod }) => /^\d{4}-\d{2}-\d{2}$/.test(lastmod))).toBe(true)
  })
  ...
  it("rejects a URL set above the sitemap protocol limit", () => {
    ...
    expect(() =>
      buildSitemap(indexFor(offers), BASE, { now: new Date("2026-08-22T00:00:00Z") }),
    ).not.toThrow()
    expect(() =>
